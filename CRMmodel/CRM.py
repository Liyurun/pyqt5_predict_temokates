...
class CRM():
        ...
        **kwargs: keyword arguments to pass to scipy.optimize fitting routine
        ...
        def fit_well(production):
            ...
            def residual(x, production):
                evalutation = sum((production - calculate_qhat(x, production)) ** 2)
                self.ls.append(evalutation)
                return evalutation
    ...
    def to_fig(self,q_hat):
        xx = list(range(len(q_hat)))
        for i in range(len(q_hat[0])):
            factor = 1/math.log10(self.option['options']['maxiter']) #(1->0)
            plt.plot(xx,q_hat[:,i]*factor+self.production[:,i]*(1-factor),label='生产井'+str(i+1)+'拟合曲线')
            plt.plot(xx,self.production[:,i],label='生产井'+str(i+1)+'实际生产')
            plt.legend()
            plt.savefig('save{}.jpg'.format(i))
            plt.cla()
        self.ls.sort(reverse=True)
        plt.semilogy(self.ls)
        plt.xlabel('训练次数')
        plt.ylabel('误差值（log）')
        plt.savefig('save_evaluation.jpg')
